<?php if(isset($this->params['named']['habtm']) && $this->params['named']['habtm'] == $habtmModel):?>
    <div style="display:<?php echo (isset($this->params['named']['habtm']) && $this->params['named']['habtm'] == $habtmModel)?'block':'none';?>;" class="etabs-content" id="<?php echo $habtmModel?>">
        <?php echo $this->AppForm->create($modelClass)?>
            ...
            <div class="powerwidget powerwidget-habtm" role="widget">
                <div role="content">
                    <!-- .toolbar -->         
                    <?php echo $this->assign('toolbar-index-bulkAction', "<option value=\"{$this->Html->url(array('action' => 'unjoin'))}\">" . __('Unjoin') . "</option>")?>
                    <?php echo $this->assign('toolbar-index-buttons', $this->Html->link(__('add') . ' <span class="plus-10 plix-10"></span>', '#', array('title' => __('Add Relationship'), 'rel' => $habtmModel, 'data-source' => "/" . Inflector::pluralize(strtolower($habtmModel)) . "/index/fkbox:habtm/habtmModel:{$modelClass}/habtmId:{$this->data[$modelClass]['id']}", 'class' => "fk-box reload button-text-icon tip-s", 'escape' => false)))?>
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($$habtmModel as $k => $v):?>
                                        <tr>
                                            ...
                                            <td><?php echo $v[$habtmModel]['name']?></td>
    ...
    <div class="modal-<?php echo $habtmModel?>" title="<?php echo __(Inflector::pluralize($habtmModel) . " not associated with the {$modelClass}")?>" style="display:none"></div>
<?php endif?>